...
I then loaded this data in R, and wrote a simple code that went through the final draft piles one by one, calculating the total number of cases that every two cards from the set (i and j) were drafted together (n_ij). This data was stored in a matrix called "pairs". 

Now, to be clear, if in one draft card i was drafted together with two copies of card j, I would count this pair twice. This is an important distinction for common cards: I did not count the number of piles that have both cards i and j in it; I counted the number of card pair that happened to be cards i and j. It's an important distinction. Also it means that n_ii is a thing, as it shows how many times card i was part of a pair with another copy of itself.
