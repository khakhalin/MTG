...
The independent variable x was constructed from one or two components: an indicator vector for the collection (with each element $x_i$ representing the number of cards type $i$ in the collection), and the optional concatenation of a collection of card features. 
