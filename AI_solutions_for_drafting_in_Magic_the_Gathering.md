...
Magic the Gathering Draft is a sub-game within a larger trading cards game, in which several players take turns to competitively pick cards from a common pool, each trying to create a powerful collection of cards (a deck), to play against other players. Here we present four diverse strategies for creating an AI drafting agent, including a primitive heuristic, an expert-tuned complex heuristic, a Naive Bases solution, and a simple neural network agent. All these drafing agents then attempt to approximate a dataset of drafts recorded from human players in a simulated drafting environment (draftsim.com). We show that … 

our drafting agents can serve as useful benchmarks for the next generation of rule-aware drafting bots. 

All code, and the original dataset, are made publicly available.
